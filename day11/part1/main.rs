...
struct Monkey {
    ...
    true_dest: usize,
    false_dest: usize,
    items_investigated: u32,
}

struct ValueDest(i32, usize);
...
impl Monkey {
	...
	for item in &self.items {
	    let value = self.do_operation(*item) / 3;
    ...
    fn do_operation(&self, value: i32) -> i32 {
    ...
    fn test_value(&self, value: i32) -> usize {
...
fn main() {
    ...
    let mut item_queue: Vec<Vec<i32>> = Vec::new();
...
fn create_monkey(monkey_input: &[String]) -> Monkey {
	...
	.split(", ")
	.map(|item| item.parse::<i32>().expect("Bad item"))
	.collect::<Vec<i32>>();
	...
	None => {
		...
		.parse::<i32>().expect("Malformed input (operation value)");
