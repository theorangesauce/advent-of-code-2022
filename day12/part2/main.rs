...
fn main() {
    ...
    let (elevation_map, _dest_point, start_point) = get_elevation_map(&input);
    ...
    let mut a_scores: Vec<u32> = Vec::new();
    for point in elevation_map.keys() {
	match elevation_map.get(point).unwrap() {
	    'a' => a_scores.push(distance_scores.get(point).unwrap_or(&5000u32).clone()),
	    _ => continue,
	}
    }
    a_scores.sort_unstable();

    println!("The best point to start from is {} steps from the endpoint", a_scores.first().unwrap());
}
