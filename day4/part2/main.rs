...
fn is_overlapping(line: Vec<&str>) -> bool {
    ...
    if (first_range.first()..=first_range.last()).contains(&second_range.first())
	|| (first_range.first()..=first_range.last()).contains(&second_range.last())
	|| (second_range.first()..=second_range.last()).contains(&first_range.first())
	|| (second_range.first()..=second_range.last()).contains(&first_range.last()) {
	    return true;
	}
