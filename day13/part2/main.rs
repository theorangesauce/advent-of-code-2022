...
fn main() {
    ...
    for pair in input.chunks(3) {
	packets.push(pair.get(0).unwrap().to_string());
	packets.push(pair.get(1).unwrap().to_string());
    }

    packets.push("[[2]]".to_string());
    packets.push("[[6]]".to_string());

    packets.sort_unstable_by(|left, right| check_packet_order(&left, &right));

    let first_idx = match packets.binary_search_by(|item| check_packet_order(&item, &"[[2]]".to_string())) {
	Ok(n) => (n + 1) as i32,
	_ => -1 as i32,
    };
    let second_idx = match packets.binary_search_by(|item| check_packet_order(&item, &"[[6]]".to_string())) {
	Ok(n) => (n + 1) as i32,
	_ => -1 as i32,
    };

    println!("The decoder key is {}", first_idx * second_idx);
}
...
fn check_packet_order(left_packet: &String, right_packet: &String) -> Ordering {
